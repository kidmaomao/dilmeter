...
// localFirstResourceHandler keeps character-condition icons available offline.
// Other resources, and condition IDs that are not bundled yet, continue to use
// the upstream resource service for forward compatibility.
func localFirstResourceHandler(localFS fs.FS, upstream http.Handler) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		localPath, ok := bundledConditionIconPath(r.URL.Path)
		if ok {
...
func bundledConditionIconPath(requestPath string) (string, bool) {
	...
	return "/condition-icons/" + parts[0] + ".png", true
}
