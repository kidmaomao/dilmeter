...
func main2(ctx context.Context) {
	...
	cfg := loadConfig()
	selectedServer, err := applyGameServer(configuredGameServer(cfg))
	if err != nil {
		messagebox(fmt.Sprintf("无法载入服务器设置：%v", err))
		return
	}
	logger.Printf("selected game server: %s, ports %s", selectedServer.Network, strings.Join(selectedServer.Ports, ","))
	...
	if replayFile := commandLineReplayFile(); replayFile != "" {
		...
		startCaptureManager(ctx, reader, cfg.AcceleratorMode)
	}
	...
	// Keep reminder timers alive while the main window is minimized or hidden
	...
	// One low-level mouse router owns reminder dragging and the SkillBar click
	// barrier so down/up pairs cannot be split between competing hooks.
