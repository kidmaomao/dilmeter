...
func parseCombatActionPacket(id uint64, msg Message) (*CombatActionPacket, error) {
	...
	if (ttype & CombatActionTypeTakeHit) != 0 {
		...
		if msg[0].Type() == MessageElemTypeFloat {
			if msg[1].Type() != MessageElemTypeFloat {
				return nil, fmt.Errorf("parseCombatActionPacket: hit wound has unexpected type %v", msg[1].Type())
			}
			if msg[2].Type() != MessageElemTypeInt {
				return nil, fmt.Errorf("parseCombatActionPacket: hit manaDamage has unexpected type %v", msg[2].Type())
			}

			v.Hit.Damage = msg[0].Data().(float32)
			v.Hit.Wound = msg[1].Data().(float32)
			v.Hit.ManaDamage = msg[2].Data().(uint32)

			msg = msg[3:]
		} else if msg[0].Type() == MessageElemTypeByte {
			// isEncrypted, encryptedDataLen, encryptedData
			// 한국 리전에서 대미지 암호화를 안하고 주는 케이스가 없을듯? flag에 대한 고려는 안해도 될듯
			if msg[1].Type() != MessageElemTypeInt {
				return nil, fmt.Errorf("parseCombatActionPacket: hit encryptedDataLen has unexpected type %v", msg[1].Type())
			}

			if msg[2].Type() != MessageElemTypeBin {
				return nil, fmt.Errorf("parseCombatActionPacket: hit encryptedData has unexpected type %v", msg[2].Type())
			}

			encryptedData := msg[2].Data().([]byte)
			damage, wound, manaDamage, _ := koreaDamageDecrypt(encryptedData)

			v.Hit.Damage = damage
			v.Hit.Wound = wound
			v.Hit.ManaDamage = manaDamage

			msg = msg[3:]
		} else {
			return nil, fmt.Errorf("parseCombatActionPacket: hit damage has unexpected type %v", msg[0].Type())
		}
