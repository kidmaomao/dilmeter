...
type webview struct {
	...
	autofocus   bool
	transparent bool
	// keepAliveWhenMinimized leaves the WebView2 controller visible while its
	// parent HWND is minimized. The child surface is still not drawn by Windows,
	// but WebView2 does not classify the renderer as explicitly hidden and pause
	// its WebSocket/timer work. We still skip Resize so a 0x0 minimized client
	// rectangle is never passed to the controller.
	keepAliveWhenMinimized bool
	minimizedAtUnixMs      int64
	maxsz                  w32.Point
	minsz                  w32.Point
	m                      sync.Mutex
...
type WebViewOptions struct {
	...
	// Transparent gives the WebView2 composition surface a fully transparent
	// default background. The host window must also support transparency.
	Transparent bool
	...
	// KeepAliveWhenMinimized keeps the renderer active while the host window is
	// minimized. Use this for monitoring windows that must continue processing
	// WebSocket events and timers in the background.
	KeepAliveWhenMinimized bool
...
func NewWithOptions(options WebViewOptions) WebView {
	...
	w.transparent = options.Transparent
	w.keepAliveWhenMinimized = options.KeepAliveWhenMinimized
	...
	if options.Transparent {
		if err := chromium.SetTransparentBackground(); err != nil {
			log.Printf("failed to enable transparent WebView2 background: %v", err)
			return nil
		}
	}
...
func wndproc(hwnd, msg, wp, lp uintptr) uintptr {
	if w, ok := getWindowContext(hwnd).(*webview); ok {
		...
		case w32.WMNCLButtonDown:
			...
			return r
		case w32.WMSize:
			w.handleWindowSize(wp)
		case w32.WMActivate:
			// LOWORD(wParam) is the activation state. HIWORD is non-zero when
			// the window is minimized, so comparing the complete value can treat
			// an inactive minimized window as active and focus its controller.
			if lowWord(wp) == w32.WAInactive {
				break
			}
			if w.autofocus {
				w.browser.Focus()
...
func (w *webview) handleWindowSize(sizeType uintptr) {
	// WebView2 explicitly recommends hiding the controller while its parent is
	// minimized and showing it again on restore. In particular, do not pass the
	// minimized parent's 0x0 client rectangle to PutBounds: some runtimes keep
	// that empty surface after the parent window is restored, producing a black
	// or blank WebView until the process is restarted.
	if lowWord(sizeType) == w32.SizeMinimized {
		w.minimizedAtUnixMs = time.Now().UnixMilli()
		if w.keepAliveWhenMinimized {
			w.browser.Eval(`window.dispatchEvent(new CustomEvent("dilmeter-host-window-state", {detail:{minimized:true}}));`)
		} else {
			_ = w.browser.Hide()
		}
		return
	...
	w.browser.Resize()
	_ = w.browser.Show()
	if w.keepAliveWhenMinimized && w.minimizedAtUnixMs > 0 {
		awayMs := time.Now().UnixMilli() - w.minimizedAtUnixMs
		w.minimizedAtUnixMs = 0
		w.browser.Eval(`window.dispatchEvent(new CustomEvent("dilmeter-host-window-state", {detail:{minimized:false,awayMs:` + strconv.FormatInt(awayMs, 10) + `}}));`)
	}
}
...
func (w *webview) CreateWithOptions(opts WindowOptions) bool {
	...
	classNameValue := "webview"
	var backgroundBrush windows.Handle
	if w.transparent {
		classNameValue = "webview-transparent"
		brush, _, _ := createSolidBrush.Call(0x000000)
		backgroundBrush = windows.Handle(brush)
	}
	className, _ := windows.UTF16PtrFromString(classNameValue)
	wc := w32.WndClassExW{
